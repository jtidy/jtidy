        ...
        if (this.configuration.CharEncoding == Configuration.LATIN1)
        {
            if (c > 255)  /* multi byte chars */
            {
                ...
                for (int i = 0; i < entity.length(); i++)
                    addC(entity.charAt(i), this.linelen++);
    ...
    */
    ...
    private void printString(Out fout, int indent, String str)
    {
        for (int i = 0; i < str.length(); i++ )
            addC(str.charAt(i), this.linelen++);
    }
    ...
    private void printAttrValue(Out fout, int indent, String value, int delim, boolean wrappable)
    {
        ...
        if (value != null)
        {
            ...
            while (i < valueChars.length)
            {
                ...
                if (wrappable && c == ' ' && indent + this.linelen < this.configuration.wraplen)
                {
                    this.wraphere = this.linelen;
                ...
                if (c == delim)
                {
                    ...
                    for (int j = 0; j < entity.length(); j++ )
                        addC(entity.charAt(j), this.linelen++);
    ...
    private void printAttribute(Out fout, int indent, Node node, AttVal attr)
    {
        ...
        for (int i = 0; i < name.length(); i++ )
            addC(Lexer.foldCase(name.charAt(i),
                            ...
                            Node node, AttVal attr)
    {
        ...
        if (this.configuration.XmlOut &&
                this.configuration.XmlSpace &&
                ParserImpl.XMLPreserveWhiteSpace(node, this.configuration.tt) &&
    ...
    */
    ...
    private void printTag(Lexer lexer, Out fout, short mode, int indent, Node node)
    {
        ...
        for (int i = 0; i < p.length(); i++ )
            addC(Lexer.foldCase(p.charAt(i),
        ...
        {
            addC(' ', this.linelen++);   /* compatibility hack */
            addC('/', this.linelen++);
        }
    ...
    private void printEndTag(Out fout, short mode, int indent, Node node)
    {
       ...
       */
if (false) {
        if (indent + this.linelen++ < this.configuration.wraplen && !((mode & NOWRAP) != 0))
            this.wraphere = this.linelen++;
}
...
if (false) { //#if 0
    ...
    private void printNavBar(Out fout, int indent)
    {
        ...
        {
            buf = "<a href=\"slide" +
                  (new Integer(this.slide - 1)).toString() +
                  ".html\">previous</a> | ";
    ...
    */
    public void printSlide(Out fout, short mode, int indent, Lexer lexer)
    {
        ...
        s = "<div onclick=\"document.location='slide" +
            (new Integer(this.slide < this.count ? this.slide + 1 : 1)).toString() +
            ".html'\">";
        ...
        if (this.slidecontent.tag == tt.tagH2)
        {
            ...
            addC(Lexer.foldCase('h',
                                     this.configuration.UpperCaseTags,
                                     this.configuration.XmlTags),
                 this.linelen++);
            addC(Lexer.foldCase('r',
                                     this.configuration.UpperCaseTags,
                                     this.configuration.XmlTags),
                 this.linelen++);
            ...
            printTree(fout, mode,
                (this.configuration.IndentContent ? indent+this.configuration.spaces : indent), lexer, this.slidecontent);
